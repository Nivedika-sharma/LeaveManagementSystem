...
namespace LeaveManagementSystem.Pages.WFH
{
    ...
    public class ApplyModel : PageModel
    {
        ...
        public void OnGet()
        {
            // if current user is not admin, pre-select current employee
            ...
            if (!string.IsNullOrEmpty(email))
            {
                ...
                if (emp != null)
                {
                    // also consider role claim
                    IsAdmin = User.IsInRole("Admin") || (emp.Status ?? string.Empty).Equals("Admin", StringComparison.OrdinalIgnoreCase);
        ...
        {
            // ensure dropdown data available for redisplay
            var email = User.Identity?.Name;
            ...
            // if current user is not admin, force EmployeeId to current user's id
            if (!string.IsNullOrEmpty(email))
            {
                var emp = Data.EmployeeDB.GetByEmail(email);
                if (emp != null)
                {
                    CurrentUserName = emp.Name;
                    if (!((emp.Status ?? string.Empty).Equals("Admin", StringComparison.OrdinalIgnoreCase) || User.IsInRole("Admin")))
                    {
                        EmployeeId = emp.Id;
                    }
                }
            ...
            Core.BusinessObject.WFH wfh = new Core.BusinessObject.WFH
            {
            ...
            OperationResult result = wfhManager.Add(wfh);

            if (result.Status == (int)OperationStatus.Success)
            {
                TempData["SuccessMessage"] = result.Message ?? "WFH request applied successfully.";
